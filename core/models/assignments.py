...
class Assignment(db.Model):
    ...
    def upsert(cls, assignment_new: 'Assignment'):
        if assignment_new.id is not None:
            ...
            assertions.assert_valid(assignment.state is AssignmentStateEnum.DRAFT,'only assignment in draft state can be edited')
    ...
    @classmethod
    def grader(cls, _id, grade, principal: Principal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(assignment.teacher_id == principal.teacher_id, 'This assignment is submitted to some other teacher')
        assertions.assert_valid(assignment.state is AssignmentStateEnum.SUBMITTED, 'assignment is not submitted yet')
        assertions.assert_valid(grade in GradeEnum.__members__,f'invalid grade passed, valid grades are {GradeEnum.__members__}')
